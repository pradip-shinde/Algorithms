...
class Tree{
	...
	class CVertical{
		public :
		CVertical(const Node*pNode,int iDist){
			m_iDist = iDist;
			m_pNode = pNode;
		}
		int m_iDist;
	...
	void VerticalOrder(const Node* pNode)
	{
		cout<< endl<< endl<< "Vertical Order Traversal:"<< endl;
		queue<CVertical> que;
		que.push(CVertical(pNode,0));
		map<int, list<const Node*>> mapLevels;
		while(!que.empty())
		{
			CVertical obj = que.front();
			que.pop();
			list<const Node*>& lst = mapLevels[obj.m_iDist];
			lst.push_back(obj.m_pNode);
			if(obj.m_pNode->pLeft != nullptr)
			{
				que.push(CVertical(obj.m_pNode->pLeft, obj.m_iDist-1));
			}
			if(obj.m_pNode->pRight != nullptr)
			{
				que.push(CVertical(obj.m_pNode->pRight, obj.m_iDist+1));
			}
			
		}
		int iLevel =1;
		for(map<int, list<const Node*>>::iterator itr = mapLevels.begin(); itr != mapLevels.end(); itr++)
		{
			list<const Node*> lst = itr->second;
			cout<<"Level "<<iLevel <<" : ";
			for(list<const Node*>::const_iterator  itrl = lst.begin(); itrl != lst.end(); itrl++)
			{
				cout<< (*itrl)->iData <<" ";
			}
			iLevel++;
